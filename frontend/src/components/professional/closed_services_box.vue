  ...
  <div class="service-list">
    ...
    <div class="service-container">
      <div class="service-header">
        ...
        <span class="header-item">Date of Completion</span>
        <span class="header-item">Remarks</span>
        <span class="header-item">Status</span>
      </div>
      <div class="service-row" v-for="request in closedRequests" :key="request.id">
        <span>{{ request.id }}</span>
        <span>{{ request.customer_name }}</span>
        <span>{{ request.customer_phone }}</span>
        <span>{{ request.customer_pincode }}</span>
        <span>{{ request.date_of_completion }}</span>
...
export default {
  ...
  methods: {
    async fetchClosedRequests() {
      try {
        const requests = await professionalService.getClosedServiceRequests();
        this.closedRequests = requests; // Store the fetched requests
      } catch (error) {
  ...
  mounted() {
    this.fetchClosedRequests(); // Fetch requests when the component is mounted
  }
};
</script>
